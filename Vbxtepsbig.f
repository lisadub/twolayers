...
c This solves npde 1D hyperbolic & parabolic PDEs
c in the form y_t=f(y,t,x,yx,yxx,yxxx,yxxxx)
...
c Note this is on a periodic domain -length to length
     ...
     + NEQ, NETF, NFCN, NOINV, NRWK, NSTEPS
      INTEGER NPDE, NPRINT
      PARAMETER (MXNEQ = 1048,MAXORD=5,n=256,npde=2,neq=npde*n)
c this has n modes and npde pdes
      PARAMETER (MXLIW =20+MXNEQ,MXLRW=40+(maxord+4)*mxneq+mxneq**2)
      INTEGER INFO(15), IWORK(MXLIW), IPAR(1)
      DOUBLE PRECISION Y(MXNEQ), YPRIME(MXNEQ), RTOL(1), ATOL(1),
     +     RWORK(MXLRW), RWK,rpar(1),length,maxzeta,diff
c compiles using the intel  and g77 compiler with no flags:
c dassl stuff
      INTEGER NPTS
      parameter(nprint=500)
      DOUBLE PRECISION XL, XR, T0, TE
      DOUBLE PRECISION  TPRINT(nprint)
      EXTERNAL RESID, DDASSL
c fft stuff
      double precision yf(mxneq)
      parameter(len=2,lenh=len/2)
...
C   Initial Yprime = 0
    ...
    1 CONTINUE
      ...
      do ii=1,nprint
         tprint(ii)=tfinal*dble(ii)/dble(nprint)
      enddo
...
C ccc Check length arrays
      ...
      IF (LIW .GT. MXLIW) THEN
         ...
         STOP 'Workspace too small'
      ENDIF
      LRW    =40+(maxord+4)*mxneq+mxneq**2
c 40+(MAXORD+4)*NEQ + neq*neq
...
c output data
         ...
         do 105 kk=1,n
            ...
            if(y(kk).lt.minH) then
               minH=y(kk)
	       xmin=yf(kk)
            endif

            if(y(kk).gt.maxH) then
               maxH=y(kk)
            endif
...
C
  900 CONTINUE
      ...
      STOP 'whoops! DASSL error'
      END

c the subroutine with the pde
      SUBROUTINE RESID(T, Y, YPRIME, DELTA, IRES, RPAR, IPAR)
      implicit double precision (a-h,o-z)
      INTEGER IRES,IPAR(1),n
      DOUBLE PRECISION T
      DOUBLE PRECISION Y(*), YPRIME(*), DELTA(*), RPAR
      parameter(mxneq=1048)
      double precision yp(mxneq), dydx(mxneq)
      parameter(len=2,lenh=len/2)
      double precision yf(mxneq)
      double precision thetas(mxneq),thetasx(mxneq),thetasxx(mxneq)
      double precision u(len,mxneq),ud(len,mxneq),dudx(len,mxneq)
     . ,d4udx4(len,mxneq),taub(mxneq),enu
     + ,uh(len,mxneq),uhx(len,mxneq) 
      double precision wc(len,mxneq),zc(len,mxneq)
     +,d2udx2(len,mxneq),d3udx3(len,mxneq)
     +,press(len,mxneq),dpdx(len,mxneq),h1(len,mxneq)
     +,fftpress(len,mxneq),ffth1(len,mxneq)
      double precision flux(len,mxneq),dfluxdx(len,mxneq)
      double precision ky,hx(mxneq),deltax(mxneq),ui(mxneq),tau(mxneq)

      double precision dd1(mxneq),dd2(mxneq),h(mxneq),q(mxneq)
     +,c(len,mxneq),dcdx(len,mxneq),E1(mxneq),E2(mxneq),E3(mxneq)
     ...
     +,ee1(mxneq),ee2(mxneq),p0x(mxneq),Vb(mxneq)
...
c         call dy2df(u,2,d2udx2,n)
       ...
       do ii=1,n
        ...
        D1(ii)=-(h(ii)-bbeta)**2d0
        D2(ii)=h(ii)+2*bbeta
        D3(ii)=(h(ii)+1d0)**2d0
        D4(ii)=h(ii)-2d0
        D5(ii)=m1*(1-h(ii)**2d0)+m2*(h(ii)**2d0-bbeta**2d0)
        D5(ii)=D5(ii)/(m1*(h(ii)+1)+m2*(bbeta-h(ii)))
        D6(ii)=m2*D1(ii)*(2d0*D2(ii)+3d0*D5(ii))
        D6(ii)=D6(ii)+m1*D3(ii)*(2d0*D4(ii)+3d0*D5(ii))
        D6(ii)=D6(ii)/(12d0*m1*m2)
...
c  horiz velocity at O(1):
        ...
        E1(ii)=h(ii)-bbeta-m1*(h(ii)+1)/m2
	E2(ii)=-(h(ii)*h(ii)-bbeta**2d0)/(2*m1)+
     + h(ii)*(h(ii)+1)/m2
	E3(ii)=(-0.5d0*(h(ii)+1d0)**2d0)/m2
	E4(ii)=-(h(ii)+1d0)*(dudx(1,ii)*(dd1(ii)+cc1)
     + +(h(ii)-bbeta)*dd1x(ii))/m2
	E5(ii)=-(dd1x(ii)*(h(ii)-bbeta)+(cc1+dd1(ii))*dudx(1,ii))

	E6(ii)=-D2(ii)*D1(ii)/(6d0*m1)
	E6(ii)=E6(ii)-(D1(ii)*E2(ii)/(2d0*E1(ii)))
	E6(ii)=E6(ii)-(0.5d0*(D3(ii))
     + *(h(ii)+(E2(ii)*m1/E1(ii))))
	E7(ii)=-E3(ii)*D1(ii)/(2d0*E1(ii))
	E7(ii)=E7(ii)+(E3(ii)*D4(ii)/3d0)
	E7(ii)=E7(ii)-0.5d0*(D3(ii)*
     +((E3(ii)*m1/E1(ii))-h(ii)))
	E8(ii)=-E4(ii)*D1(ii)/(2d0*E1(ii))
	E8(ii)=E8(ii)-(0.5d0*D3(ii)*
     + ((E4(ii)*m1/E1(ii))+E5(ii)))
	...
	f1(ii)=(E2(ii)*p1x(ii)+E3(ii)*p2x(ii)+E4(ii)*(q(ii)+qbar))/E1(ii)
...
610   WRITE (XERN3, '(1P,D15.6)') TN
      CALL XERMSG ('SLATEC', 'DDASSL',
     *   'AT CURRENT T = ' // XERN3 // ' 5000 STEPS TAKEN ON THIS ' //
...
c this does differentiation using FFT
      subroutine dy2df(u,nderiv,ud,n)
      implicit double precision (a-h,o-z)
      parameter(len=2,mxneq=1048)
      parameter(lenh=len/2)
c      parameter(n=512)
c modes
c      parameter(np=n+1)
      double precision u(lenh,2,mxneq),ud(lenh,2,mxneq)
      double precision wc(lenh,2,mxneq),zc(lenh,2,mxneq)
      common /yfac/trigy(2,mxneq),nfay,ifay(20)
      common/wvz/wavey1(mxneq),wavey2(mxneq),modk(mxneq)
cc,wavey3(n),wavey4(n)
c,wavey0(n)
cc      common/bigwork/wc,zc
      np=n+1
      do 40 k=1,np
         do 30 ij=1,lenh
            wc(ij,1,k)=u(ij,1,k)
 ...
 80            continue
      ...
      implicit double precision (a-h,o-z)
      double precision a(len,2,0:n-1),c(len,2,0:n-1)
...
c      data pi/3.1415926535898/
 ...
 50       continue
      ...
      implicit double precision (a-h,o-z)
c      parameter(root=0.7071067811865)
c      parameter(asn60=0.5*1.732050807569)
      double precision a(len,2,0:n-1),c(len,2,0:n-1),trig(2,0:n-1)
